...
from modules.contexts.schemas import CreateContextRequest, UpdateContextRequest
...
async def get_all_contexts(
    ...
    return await conn.fetch(
        """
        SELECT
            c.id, c.name, c.context_type, c.platform, c.priority, c.notes, c.is_active, c.created_at, c.updated_at,
            ...
            COALESCE(SUM(a.payment_kes) FILTER (WHERE a.paid_at IS NOT NULL AND a.is_active = TRUE AND a.tenant_id = $1), 0) AS total_earnings,
            COALESCE(SUM(a.payment_kes) FILTER (WHERE a.status = 'Submitted' AND a.paid_at IS NULL AND a.payment_kes IS NOT NULL AND a.is_active = TRUE AND a.tenant_id = $1), 0) AS unpaid_kes,
            COUNT(a.id) FILTER (WHERE a.status = 'Submitted' AND a.paid_at IS NULL AND a.payment_kes IS NOT NULL AND a.is_active = TRUE AND a.tenant_id = $1) AS unpaid_count
        FROM contexts c
        LEFT JOIN assignments a ON c.id = a.context_id
        WHERE c.tenant_id = $1
...
async def create_context(
    ...
    return await conn.fetchrow(
        """
        INSERT INTO contexts (tenant_id, name, context_type, platform, priority, notes)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id, name, context_type, platform, priority, notes, is_active, created_at, updated_at
        """,
        tenant_id,
        data.name,
        data.context_type,
...
async def update_context(
    ...
    if not fields:
        return await conn.fetchrow(
            """
            SELECT id, name, context_type, platform, priority, notes, is_active, created_at, updated_at
            FROM contexts
            WHERE id = $1
              AND tenant_id = $2
              AND is_active = TRUE
...
async def get_financial_analytics(
    ...
    row = await conn.fetchrow(
        """
        SELECT
            COALESCE(SUM(payment_kes) FILTER (WHERE paid_at IS NOT NULL), 0)                                                  AS total_collected_kes,
            COALESCE(SUM(payment_kes) FILTER (WHERE status = 'Submitted' AND paid_at IS NULL AND payment_kes IS NOT NULL), 0) AS pending_payout_kes,
