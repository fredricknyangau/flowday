...
export function ClientStatementModal({ client, isOpen, onClose }: Props) {
  ...
  const clientAssignments = assignments.filter((a) => {
    if (a.client_id !== client.id || a.status === 'Cancelled') return false
    if (filterMode === 'submitted') return a.status === 'Submitted'
    return true
  })

  const totalWords = clientAssignments.reduce((sum, a) => sum + (a.word_count ?? 0), 0)
  const totalDueKES = clientAssignments.reduce(
    (sum, a) => sum + (a.payment_kes ? parseFloat(a.payment_kes) : 0),
    0
  )

  const generateStatementText = () => {
    let text = `📄 STATEMENT OF ACCOUNT — ${client.name.toUpperCase()}\n`
    ...
    if (clientAssignments.length === 0) {
      ...
      clientAssignments.forEach((a, idx) => {
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl max-w-lg w-full p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 p-0.5 rounded-lg text-xs">
            ...
            >
              All Tasks ({clientAssignments.length})
            </button>
          </div>
